...
int main() {
       ...
       cansec.EncryptMessage(my_counter, 1, cipher_counter, cipher_counter_len, tag);

       std::cout << "cipher_counter_len : " << cipher_counter_len << std::endl;
       int plaintext_len = 0;

       cansec.DecryptMessage(cipher_counter, cipher_counter_len, plaintext, plaintext_len, tag);
